...
class SessionTable(DataTable):
    ...
    def on_mount(self) -> None:
        ...
        self.add_column("Repo", key="repo", width=26)
    ...
    def set_sessions(self, sessions: list[FleetSession]) -> None:
        ...
        for i, s in enumerate(sessions):
            ...
            attention = Text("!", style="bold yellow") if s.needs_attention else Text(" ")
            ...
            row_key = self.add_row(
                attention,
                icon,
                Text(s.repo[:25], style="bold"),
                status,
                Text(detail),
                Text(format_age(s.age_seconds), style="dim"),
                key=s.session_id,
            )
